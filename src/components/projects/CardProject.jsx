        ...
        ? <>
          ...
          <div className='flex items-end justify-center h-full gap-2'>
            ...
            </button>
            {
                type.map((t,i)=> {
                    return (
                        <div key={i} onClick={() => setShowDescription(p => !p)} className='cursor-pointer border-[6px] shadow-2xl/80  border-red-950/70 border-b-[4px] border-t-[4px] border-b-black/60 border-t-black/60 rounded z-2 vt323-regular text-shadow-2xs text-shadow-black text-2xl text-red-800 bg-white px-2 py-1'>
                        {t}
                        </div>
                    )
                })
